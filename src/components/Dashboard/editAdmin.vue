<template>
  <div class="parent position-relative">
    ...
    <div class="container p-4">
      <h3 class="text-end fw-bold text-white">تعديل بيانات المشرف</h3>
      <hr />
      <div class="row justify-content-between">
        <div class="col-lg-12 border-top pt-3">
          <form @submit.prevent>
            ...
            <h4 class="text-white text-end mb-3 mt-3">
              الصلاحيات ({{ powers }})
            </h4>
            <select
              v-model="powers"
...
export default {
  ...
  mounted() {
    let result = axios
      .get(`http://localhost:3000/admins/${this.adminId}`)
      .then((response) => {
        this.name = response.data.adminName;
        this.email = response.data.adminEmail;
        this.adminPhoto = response.data.adminPhoto;
        this.powers = response.data.supervisorStatus;
      });
  },
  methods: {
    editAdmin() {
